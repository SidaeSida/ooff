...
function setQuery(router: any, pathname: string, prev: URLSearchParams, patch: Record<string, string | undefined>) {
...
export default function FilmsPage() {
  ...
  // 옵션 소스
  const editionEntries = useMemo(() => (edition === 'all' ? entries : entries.filter(e => e.editionId === edition)), [edition]);
  ...
  const availableDates = useMemo(() => {
    const set = new Set<string>();
    for (const e of editionEntries) for (const s of (screeningsByEntry.get(e.id) ?? [])) set.add(ymd(s.startsAt));
    return Array.from(set).sort();
  ...
  const filteredFilmIds = useMemo(() => {
    ...
    if (dateSet.size) {
      ...
      for (const e of es) if ((screeningsByEntry.get(e.id) ?? []).some(s => dateSet.has(ymd(s.startsAt)))) ok.add(e.id);
      ...
      <div className="bg-white border rounded-lg p-3 space-y-3">
        ...
        {(sectionCsv || dateCsv || genreCsv || q) && (
          <div>
            <button
              className="text-xs underline text-gray-600 whitespace-nowrap"
              onClick={() => setQuery(router, pathname, search, { section: undefined, date: undefined, genre: undefined, q: undefined })}
            >
              Clear all
            </button>
          </div>
